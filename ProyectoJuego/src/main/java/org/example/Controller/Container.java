...
import org.example.Components.Bullet;
import org.example.Components.EnemyShip;
import org.example.Components.SuperEnemy;
import org.example.Components.Hero;
import java.awt.*;
...
public class Container  {
    ...
    private void handleEnemyDestroyed() {
        enemiesDestroyed++;
        if (enemiesDestroyed == 5 && currentLevel==1) {
            currentLevel++;
            levelMessageShown = true;
            resetEnemySpawn();
        } if (enemiesDestroyed == 10 &&currentLevel==2) {
            currentLevel++;
            resetEnemySpawn();
        } if (currentLevel==3 && enemiesDestroyed==1){
            gameWinner = true;
        }
    }
